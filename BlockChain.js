      ...
      self.getBlockHeight().then(function(height) {
        ...
        if (height === 0) {
          ...
          self.bd.addDataToLevelDB(JSON.stringify(genesisBlock)).then(function(result) {
              ...
              self.bd.addDataToLevelDB(JSON.stringify(newBlock)).then(function(result) {
                  resolve(`addBlock: New block successfully inserted after creating genesis block: ${JSON.stringify(newBlock)}`);
        ...
        // Genesis block already exists, so simply add newBlock
            ...
            self.getBlock(height - 1).then(function(previousBlock) {
                ...
                self.bd.addDataToLevelDB(JSON.stringify(newBlock)).then(function(result) {
                    resolve(`addBlock: New block successfully inserted without creating genesis block: ${JSON.stringify(newBlock)}`);
                  }, function(error) {
